                    ...
                    while(getline(&instruction,size,fbuild) != -1)
                    {
                        ...
                        if (*instruction != '\n') 
                        {
                            ...
                            if (op != NULL) {
                                ...
                                else if (!strcmp(op, "LDX")) {
                                    //Load X register
                                    op = strtok(NULL, " ");
                                    LDX(assembly, op, &LineIndex);
                                } 
                                else if (!strcmp(op, "LDY")) {
                                    //Load Y register
                                    op = strtok(NULL, " ");
                                ...
                                else if (!strcmp(op,"ASL")) {
                                    //Shift bits to 1 left
                                    op = strtok(NULL, " ");
                                    ASL(assembly,op,&LineIndex);
                                }
                                else if (strcmp_singular(op,"INX","INX\n")) {
                                    //Increment X register
                                    assembly[LineIndex] = 0xE8;
                                }
                                else if (strcmp_singular(op,"INY","INY\n")) {
                                    //Increment Y register
                                    assembly[LineIndex] = 0xC8;
                                }
                                else if (strcmp_singular(op,"DEX","DEX\n")) {
                                    //Decrement X register
                                    assembly[LineIndex] = 0xCA;
                                }
                                else if (strcmp_singular(op,"DEY","DEY\n")) {
                                    //Decrement Y regsiter
                                    assembly[LineIndex] = 0x88;
                                }
                                else if (strcmp_singular(op,"TAX","TAX\n")) {
                                    //Transfer A to X register
                                    assembly[LineIndex] = 0xAA;
                                }
                                else if (strcmp_singular(op,"TXA","TXA\n")) {
                                    //Transfer X to A register
                                    assembly[LineIndex] = 0x8A;
                                }
                                else if (strcmp_singular(op,"TAY","TAY\n")) {
                                    //Transfer A to Y register
                                    assembly[LineIndex] = 0xA8;
                                }
                                else if (strcmp_singular(op,"TYA","TYA\n")) {
                                    //Transfer Y to A register
                                    assembly[LineIndex] = 0x98;
                                }
                                else if (strcmp_singular(op,"NOP","NOP\n")) {
                                    //NOP instruction
                                    assembly[LineIndex] = 0xEA;
                                }
                                else if (strcmp_singular(op,"TXS","TXS\n")) {
                                    //Transfer X to Stack Pointer
                                    assembly[LineIndex] = 0x9A;
                                }
                                else if (strcmp_singular(op,"TSX","TSX\n")) {
                                    //Transfer Stack Pointer to X
                                    assembly[LineIndex] = 0xBA;
                                }
                                else if (strcmp_singular(op,"PHA","PHA\n")) {
                                    //Push accumulator to stack
                                    assembly[LineIndex] = 0x48;
                                }
                                else if (strcmp_singular(op,"PLA","PLA\n")) {
                                    //Pull accumulator from stack
                                    assembly[LineIndex] = 0x68;
                                }
                                else if (!strcmp(op,"CMP")) {
                                    //Compare Instruction (will set bits in Stauts Register)
                                    op = strtok(NULL, " ");
                                    CMP(assembly, op, &LineIndex);
                                }
                                else if (!strcmp(op,"CPX")) {
                                    //Compare Instruction X reg (will set bits in Stauts Register)
                                    op = strtok(NULL, " ");
                                    CPX(assembly, op, &LineIndex);
                                }
                                else if (!strcmp(op,"CPY")) {
                                    //Compare Instruction Y reg (will set bits in Stauts Register)
                                    op = strtok(NULL, " ");
                                    CPY(assembly, op, &LineIndex);
                                }
                                else if (!strcmp(op,"AND")) {
                                    op = strtok(NULL, " ");
                                    AND(assembly, op, &LineIndex);
                                }
