...
void _cmp_instruction(REGISTER* r, uint8_t mems);
...
void ASM(int Line, uint8_t memory[ASM_MEMORY], REGISTER* reg, uint8_t* monitorMem) {
    ...
    while (reg->PC < ROM) {
        ...
        switch (monitorMem[reg->PC]) {
            ...
            case 0xC9:
                reg->PC++;
                _cmp_instruction(reg, monitorMem[reg->PC]);
                break;
            case 0xC5:
                reg->PC++;
                _cmp_instruction(reg, monitorMem[monitorMem[reg->PC]]);
                break;
            case 0xCD:
                _cmp_instruction(reg, monitorMem[Bit16Adr(reg, monitorMem)]);
                break;
            //JMP
            case 0x4C:
                reg->PC++;
